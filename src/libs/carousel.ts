...
export function initInfiniteCarousel(options: InfiniteCarouselOptions = {}): void {
	...
	const { carouselSelector = '#carousel', trackSelector = '#slide', moveIndicatorSelector = '#move', minDesktopWidth = 768, speedPxPerSecond = 120 } = options
	...
	const setup = () => {
		...
		carousel.addEventListener('pointermove', (e) => {
			...
			if (!isDragActive && Math.abs(walk) > dragThreshold) {
				activateDragMode(e.pointerId)
				stopAutoplay()
			}
